    ...
    <SlideShell
      ...
      title={
        <span style={{ wordBreak: "keep-all" }}>
          이 역할을 할 수 있는 주체 ={" "}
          <span className="text-accent">Big Tech</span>
        </span>
      }
      accent="accent"
    >
      <div className="flex flex-1 flex-col justify-center gap-6">
        ...
        <Reveal>
          <div className="flex flex-wrap items-center justify-center gap-3">
            {enablers.map((e) => (
              <span
                key={e}
                className="rounded-full border border-border bg-surface-1 px-4 py-1.5 text-fg-muted text-[32px]"
              >
                {e}
              </span>
            ))}
          ...
          {capabilities.map((c, i) => (
            <Reveal key={c.en} when={step >= 1} delay={i * 0.1} y={18}>
              ...
              >
                ...
                <div
                  className="relative mt-3 text-h3 font-bold"
                  style={{ color: c.color }}
                >
                  {c.en}
                </div>
                <div className="relative text-micro font-semibold text-fg-dim">
                  {c.ko}
                </div>
                <div
                  className="relative mt-2 text-caption text-fg-muted leading-snug"
        ...
        <Reveal when={step >= 2} delay={0.1} className="mt-10">
          <div
          ...
          >
            ...
            >
              ...
              <span className="text-accent">
                Distribution · Monetization · Trust · Interface
              </span>{" "}
              역량으로 대중의{" "}
              <Highlight
                when={step >= 2}
